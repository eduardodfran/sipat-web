...
export function TopHazardsList({ potholes }: { potholes: Pothole[] }) {
    ...
    return (
      <div className="p-3">
        ...
        <div className="mt-3 flex h-32 items-center justify-center">
          <p className="text-[9px] text-text-muted">No hazards yet</p>
        </div>
      </div>
    )
  }
  ...
  return (
      <div className="p-4">
        ...
        <p className="text-[10px] text-text-muted">By detections</p>
        <div className="mt-3 space-y-1.5">
        {ranked.map((p, i) => {
          const pct = maxHits > 0 ? (p.total_detection_hits / maxHits) * 100 : 0
          return (
            <div key={p.pothole_id} className="flex items-center gap-2">
              <span className="w-3 text-right text-[8px] font-bold text-text-muted">{i + 1}</span>
              <div className="h-1.5 w-1.5 shrink-0 rounded-full" style={{ backgroundColor: severityColors[p.worst_severity] }} />
              <div className="min-w-0 flex-1">
                <div className="flex items-center justify-between">
                  <span className="text-[9px] text-text-secondary truncate">#{p.pothole_id}</span>
                  <span className="text-[9px] font-bold text-text-primary">{p.total_detection_hits}</span>
                </div>
